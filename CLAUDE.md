...
- **Mojo version pin** lives in `pixi.toml`. As of 2026-09-19 this is **stable Mojo 1.1.0** (`max = "==26.6.0"`, from the stable `https://conda.modular.com/max/` channel) rather than a nightly, matching napi-mojo 0.16.0 (which pins `max = "==26.6.0"`), which pins stable releases for exactly the reason below. `scripts/update-mojo-version.sh` rewrites only the version line, so repoint `channels` too if you ever move back to nightlies.
...
- **A `.node` is not self-contained, so containers need bundling.** It carries hard
  ...
  `libAsyncRTMojoBindings`, ...) that resolve only inside the pixi env. Copied bare into a
  slim image it dies at `require()` with `ERR_DLOPEN_FAILED`. `Dockerfile` runs napi-mojo's
  `scripts/bundle-runtime.sh` per addon (needs `patchelf` on Linux) and then copies the
  **whole** `build/` directory — copying only the `.node` orphans its sibling libraries and
  reproduces the failure. The closure is platform-dependent, so never hardcode the list.
  Since napi-mojo 0.14.0 the bundle **no longer includes `libstdc++`/`libgcc_s`** on Linux,
  so the runtime image must provide them. `node:22-slim` does (Node links both); Bun's
  distroless and Deno's alpine images do not — see napi-mojo's `docs/TROUBLESHOOTING.md`.
  Local `npm test` cannot catch any of this: it runs with the pixi env still on the library
  search path. Only running the container proves it.
- **`pixi.toml` platforms** are `osx-arm64`, `linux-64` and `linux-aarch64`. The last is what
  `docker run` gives you on an Apple Silicon Mac; MAX 26.6.0 and mojo-compiler 1.1.0 both
  publish it, so the container builds natively rather than under emulation.
- **`pixi.lock` is tracked** (since the Mojo 1.1.0 bump), so `pixi.toml` and the lock move together and a clone resolves the same toolchain this repo was verified against. It was git-ignored before that, which meant the pin in `pixi.toml` was the only record and every machine re-resolved independently; napi-mojo had already learned this the hard way, where a lock-less bump took its Nightly Canary down for two weeks. **Bump the two together** — a toolchain change that edits `pixi.toml` and not the lock is the failure mode this exists to prevent. Regenerate with `pixi install` (or `scripts/update-mojo-version.sh`, which runs it) and commit both.
- **`.last-good-nightly` is a hand-maintained note, not machinery.** Nothing in the repo reads or writes it — no script, workflow, or doc — so it drifts silently and has done so more than once. Despite the name it now records the last *verified toolchain*, nightly or stable (currently `26.6.0`, i.e. Mojo 1.1.0). **`pixi.toml` is the authoritative pin**; if the two ever disagree, `pixi.toml` is right.
...
- **`packages/embed` device selection, and why a green embed test used to prove nothing.** `embed.py`'s `_pick_device` tries `driver.Accelerator()` and **silently falls back to `driver.CPU()`** on failure. The roundtrip still clears its cosine gate on CPU and the demo still runs, so a GPU regression reported `PASS` — including on a paid H100 run, because `verify-all.sh` prints only `PASS` and swallows the warning. **`scripts/verify-all.sh` requires a GPU by default on every platform** (`EMBED_REQUIRE_GPU`, opt out with `EMBED_REQUIRE_GPU=0`), so neither a pod nor a laptop run can silently degrade. It deliberately does *not* sniff for `nvidia-smi`: that was the original rule and it went stale within one release when 26.6.0 gave Apple silicon a working `Accelerator()`, leaving Darwin classified as "no GPU expected". The default sets policy for **that harness only** — running `test-roundtrip.js` directly, or other consumers like `examples/semnotes`, still get the library default of "fallback allowed"; export `EMBED_REQUIRE_GPU=1` yourself to hold those to the same standard.
  **M4 GPU: real but parity-class, and shape-dependent (2026-09-19, MAX 26.6.0).** This used to read "`Accelerator()` init returns 'Not implemented for device: Apple M4'". On 26.6.0 `driver.Accelerator()` **succeeds** and returns `Device(type=gpu,id=0)`, so the old hard failure and its CPU fallback are gone. Measured on an M4 (`execute` + explicit `synchronize()`, n=8): the GPU **wins at the shapes this workload uses** — 100×14, the shape `bench.js` reports, is 25.75 ms gpu vs 30.74 ms cpu (**1.19×**), and 8×14 through 128×64 all favour the GPU by 1.04–1.30×. CPU wins only at the extremes: 1×14 (1.54×, dispatch-bound) and 256×128 (1.16×, where O(S²) attention scales worse). **The crossover tracks sequence length, not batch size** — chunking a 256×128 batch at the same seq_len does not recover it. `device="gpu"` is therefore the correct default here.
  **But parity-class is the ceiling, not a stepping stone.** 1.0–1.3× is nothing like H100, so benchmarking and deployment stay on NVIDIA; M4 is for correctness work and local iteration. Three tuning levers were measured and are all dead ends on M4: **fp16** (0.95×, no gain — so arithmetic throughput is not the constraint), **batch chunking** (above), and **zero-copy D2H** (already 0.4–0.8 ms via DLPack against ~900 ms of compute, so `embed.py`'s "Phase 2 zero-copy" note has nothing to win here — make that case on H100/PCIe or not at all).
  A consequence for `EMBED_REQUIRE_GPU`: on Darwin it proves the device initialized and that you are on the faster of the two paths, but with a small margin — do not read a green Darwin run as evidence of GPU-class speedup. On NVIDIA it means what it says. Set `EMBED_REQUIRE_GPU=0` to unblock yourself if MAX ever regresses Apple-silicon init.
- **Node version**: `engines.node >=22.12.0` at the repo root and `>=22.12` in both
  packages. This is a hard floor, not a preference — napi-mojo (0.13 and later) resolves N-API symbols
  at load time that Node 20 does not export, so `require()` of any addon throws there. CI
  and the Dockerfile are both on Node 22 for that reason.
- **Build outputs are git-ignored** (`build/*.node`). Don't commit them. "Fixtures" is *not* a blanket rule despite what this line used to say: `.gitignore` covers only `examples/rag-demo/fixtures/*.bin`, while `examples/matmul/fixtures/matmul_dyn.onnx` is tracked. `packages/embed/fixtures/` is generated by `packages/embed/reference.js` — a fresh clone has none, and `test-roundtrip.js` fails until it runs. `scripts/verify-all.sh --with-embed-test` generates them when absent, and they are now git-ignored so a `git add -A` after a run cannot sweep 150 KB of regenerable float32 into a commit.
