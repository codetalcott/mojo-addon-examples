...
- **`packages/embed` device selection, and why a green embed test used to prove nothing.** `embed.py`'s `_pick_device` tries `driver.Accelerator()` and **silently falls back to `driver.CPU()`** on failure. The roundtrip still clears its cosine gate on CPU and the demo still runs, so a GPU regression reported `PASS` — including on a paid H100 run, because `verify-all.sh` prints only `PASS` and swallows the warning. **`scripts/verify-all.sh` requires a GPU by default on every platform** (`EMBED_REQUIRE_GPU`, opt out with `EMBED_REQUIRE_GPU=0`), so neither a pod nor a laptop run can silently degrade. It deliberately does *not* sniff for `nvidia-smi`: that was the original rule and it went stale within one release when 26.6.0 gave Apple silicon a working `Accelerator()`, leaving Darwin classified as "no GPU expected". The default sets policy for **that harness only** — running `test-roundtrip.js` directly, or other consumers like `examples/semnotes`, still get the library default of "fallback allowed"; export `EMBED_REQUIRE_GPU=1` yourself to hold those to the same standard.
  **M4 still has no useful GPU path — but it now fails differently (2026-09-19, MAX 26.6.0).** This used to read "`Accelerator()` init returns 'Not implemented for device: Apple M4'". On 26.6.0 `driver.Accelerator()` **succeeds** on an M4 and returns `Device(type=gpu,id=0)`, so the old hard failure and its CPU fallback are gone. **That is a change of mechanism, not of capability.** Measured on an M4 (n=6 after warmup, all-ones input): 32×32 batch 20.1 ms gpu vs 22.9 ms cpu (1.14×), 64×64 87.7 vs 91.4 (1.04×), 256×128 **862 vs 626 — 0.73×, i.e. slower than CPU** — and latency scales linearly with batch, which is CPU-like. Correctness is unaffected (cosine 1.000000). So **embed GPU iteration still happens on RunPod**; do not read a green Darwin embed run as evidence of a working GPU path.
  A consequence for `EMBED_REQUIRE_GPU`: on Darwin the requirement is satisfied by `Accelerator()` merely *initializing*, so there it proves the device came up, not that anything was accelerated. On NVIDIA it means what it says. The guard is still worth having on both — it catches a genuine init failure — but weight the Darwin signal accordingly, and set `EMBED_REQUIRE_GPU=0` to unblock yourself if MAX ever regresses Apple-silicon init.
