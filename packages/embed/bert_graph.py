...
VENDORED (Apache-2.0) from:
  https://github.com/modular/modular/tree/main/max/python/max/pipelines/architectures/bert/graph.py

Why: importing `max.pipelines.architectures.bert` pulls in the full
`max.pipelines.lib` framework (pydantic, msgspec, pillow, and a growing list
of deps for other model types — diffusion, vision, etc.). We only need the
BERT graph builder. Vendoring + a minimal duck-typed `BertModelConfig` keeps
the spike's dependency surface small.
...
# Vendored (Apache-2.0). Replaces the upstream BertModelConfig import which
# transitively pulls max.pipelines.lib -> pydantic/msgspec/pillow. BertModel
# only actually reads these four fields, so a minimal duck-typed dataclass
# is sufficient.
...
class BertModelConfig:
    huggingface_config: Any  # transformers.AutoConfig
    dtype: DType
    device: DeviceRef
